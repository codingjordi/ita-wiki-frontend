...
export const useGetBookmarksList = (resources: IntResource[]) => {
    ...
    const [filteredResources, setFilteredResources] = useState<IntBookmarkElement[]>([]);
    ...
    useEffect(() => {
        ...
        const fetchBookmarks = async () => {
            try {
                const fetchedBookmarks: Bookmark[] = await getBookmarks(user.id.toString());

                const filteredBookmarks: IntBookmarkElement[] = resources
                    .filter((resource) =>
                        fetchedBookmarks.some((bookmark) => bookmark.resource_id === resource.id)
                    )
                    .map((resource) => {
