...
describe("BookMarkList Component", () => {
  ...
  it("should render BookmarkComponent for each bookmark", async () => {
    ...
    await waitFor(() => {
      ...
      mockBookmarks.forEach((bookmark) => {
        expect(
          screen.getByText(`Título: ${bookmark.title}`),
        ).toBeInTheDocument();
        expect(
          screen.getByText(`Descripción: ${bookmark.description}`),
        ).toBeInTheDocument();
      });
    });
  });
  ...
  it("should display the correct props in BookmarkComponent", async () => {
    ...
    await waitFor(() => {
      mockBookmarks.forEach((bookmark) => {
        expect(
          screen.getByText(`Título: ${bookmark.title}`).closest("a"),
        ).toHaveAttribute("href", bookmark.url);
      });
    });
  });
});
