...
describe("getResources", () => {
  it("debería lanzar un error si fetch falla", async () => {
    ...
    global.fetch = vi.fn(() => Promise.reject(new Error("Error al obtener los recursos")));
  ...
  it("debería devolver los datos de la API cuando la respuesta es exitosa", async () => {
    ...
    const mockData = [{ id: 1, name: "Recurso 1" }, { id: 2, name: "Recurso 2" }];
    ...
    expect(fetch).toHaveBeenCalledWith(expect.stringContaining("resources/lists"));
  });
